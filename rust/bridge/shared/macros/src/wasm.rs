...
fn bridge_fn_sync(
    name_with_prefix: &Ident,
    _orig_name: &Ident,
    _input_args: &[(&Ident, &Type)],
    _result_kind: ResultKind,
) -> TokenStream2 {
...
fn bridge_fn_async(
    ...
    quote! {
        #[wasm_bindgen]
        pub(crate) async fn #name_with_prefix() -> Result<JsValue, JsValue> {
...
pub(crate) fn bridge_fn(name: String, sig: &Signature, result_kind: ResultKind) -> TokenStream2 {
    ...
    let implementation = match sig.asyncness {
        Some(_) => bridge_fn_async(&name_with_prefix, &sig.ident, &input_args, result_kind),
        None => bridge_fn_sync(&name_with_prefix, &sig.ident, &input_args, result_kind),
    };
    ...
    quote! {
        ...
        // wasm_register!(#name_without_prefix);
    }
}
